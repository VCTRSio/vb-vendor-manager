 ...
 */
function vmRunTool(string $tool, array $args): array
{
    $proc = new Process([PHP_BINARY, vmSrc().'/tools/'.$tool, ...$args]);
    $proc->run();
...
it('is byte-compatible: ArtifactSigning::signBytes ↔ tools/verify.php', function () {
    expect(is_file(vmSrc().'/tools/verify.php'))->toBeTrue();
